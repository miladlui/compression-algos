...
static const unsigned char ELIAS_GAMMA_MAGIC[4] = {'E', 'G', 'A', 'M'};
...
static int elias_gamma_compress(
    ...
    size_t *output_size)
{
    ...
    memcpy(out, ELIAS_GAMMA_MAGIC, 4);
...
static int elias_gamma_decompress(
    const unsigned char *input,
    size_t input_size,
    unsigned char **output,
    size_t *output_size)
{
    if (input_size < 4 || memcmp(input, ELIAS_GAMMA_MAGIC, 4) != 0)
        return -1;
    BitReader r;
...
Compressor elias_gamma_compressor = {
    .name = "elias_gamma",
    .compress = elias_gamma_compress,
    .decompress = elias_gamma_decompress};